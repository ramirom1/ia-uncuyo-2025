...
class QAgent(BaseAgent):
    ...
    def max_action_q(self, observation:Observation) -> float:
        ...
        max_q = float("-inf")
        for a in actions:
            q = self.q_values.get((state_id, a), 0.0)
            if q > max_q:
                max_q = q

        # Si nunca se seteó nada y no había entradas, max_q será -inf → devolvemos 0.0 por defecto
        return 0.0 if max_q == float("-inf") else max_q
    ...
    def select_action(self, observation:Observation, testing=False) -> tuple:
        ...
        epsilon = 0.0 if testing else self.current_epsilon

        # ¿Exploramos?
        if (not testing) and (random.uniform(0.0, 1.0) < epsilon):
            action = random.choice(actions)
        else:
            # Explotación: elegir la(s) acción(es) con Q máximo
            # Empatamos al azar entre las mejores para evitar sesgos sistemáticos
            q_vals = [(a, self.q_values.get((state_id, a), 0.0)) for a in actions]
            max_q = max(q for _, q in q_vals)
            best_actions = [a for a, q in q_vals if q == max_q]
            action = random.choice(best_actions)

        # Inicializar el par (estado, acción) si no existe
    ...
    def play_game(self, observation, episode_num, testing=False):
        ...
        while not observation.end:
            ...
            if not testing:
                ...
                # Si el episodio terminó, no hay bootstrap del siguiente estado
                if observation.end:
                    target = observation.reward
                else:
                    target = observation.reward + self.gamma * self.max_action_q(observation)
